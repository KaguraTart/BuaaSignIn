...
<details class="collapse"><summary>使用方法</summary>
<p>1. 填学号 + 统一认证密码，点击「查询课程」；或粘贴已登录的 loginName 跳过密码。</p>
<p>2. 从列表中选择要签到的课程（开始前 10 分钟 至 结束），点击「签到」。</p>
<p>3. 浏览器会记住登录态，下次打开自动恢复，过期了再输密码。</p>
</details>
<details class="collapse"><summary>如何获取已登录的 loginName（不想输密码时）</summary>
<p>1. 浏览器打开 <a href="https://iclass.buaa.edu.cn:8346/?type=jumpMyCenter" target="_blank" rel="noopener">https://iclass.buaa.edu.cn:8346/?type=jumpMyCenter</a>，会跳到统一认证登录页。</p>
<p>2. 填学号密码登录，登录成功后浏览器会自动跳回智慧教室，地址栏 URL 会变成类似：</p>
<p style="word-break:break-all;background:#f8f9fa;padding:6px 8px;border-radius:6px;font-family:monospace;font-size:12px;">https://iclass.buaa.edu.cn:8346/?<b>loginName=RkU4M0MwNEVCQjUzOEQzQjc0NjZGRDhBNzhEOUQzMDQ=</b>&amp;type=jumpMyCenter#/MyCenter</p>
<p>3. 复制 <code>loginName=</code> <b>后面</b>那串字符（到 <code>&amp;</code> 之前），粘到本页「已登录的 loginName」框。</p>
<p>4. 这种方式无需把密码交给本工具，但 loginName 有效期约半天，过期了要重新走一遍。</p>
</details>
<details class="collapse"><summary>🔒 数据安全 / 隐私</summary>
<p>· 所有数据（学号、sessionId、loginName）<b>只存在你浏览器的 localStorage</b>，不会上传到任何第三方服务器。</p>
<p>· 统一认证密码 <b>不保存</b>，登录成功后立即丢弃；本工具后台只把它透传给 sso.buaa.edu.cn 一次。</p>
<p>· 所有请求只在本机 proxy.js (localhost:8787) 和 BUAA iClass 内网之间进行，没有日志上报、没有远程存储。</p>
<p>· 点页面底部「退出登录」可立即清空本机记忆。</p>
</details>
...
<div class="form-group">
  <label>已登录的 loginName（可选）</label>
  <input id="sloginname" placeholder="从 iClass 登录后的 URL 里复制 loginName= 后面的值" autocomplete="off" spellcheck="false">
  <div style="font-size:12px;color:var(--muted);margin-top:6px;line-height:1.6">
    💡 不知道怎么获取？点上面 <a href="#" onclick="event.preventDefault();const d=document.querySelectorAll('details.collapse');for(const x of d){if(x.querySelector('summary').textContent.includes('loginName')){x.open=true;x.scrollIntoView({behavior:'smooth',block:'center'});}}" style="color:var(--t)">「如何获取已登录的 loginName」</a> 查看教程
  </div>
</div>
